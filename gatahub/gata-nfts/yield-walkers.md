# Yield Walkers

{% hint style="info" %}
Total reward Distributed : **12750 PHOTON, 0.6K USD**\
Total Epoch Distributed:  **6 Epoch**\
\
(_Last updated: 30 March 2026)_\
_For Epoch details_ [↗](https://github.com/ABCharlieEth/snapshots/tree/main/Yield%20Paws)
{% endhint %}

## About the collection

**Collection name:** [Yield Walkers](https://stargaze.zone/m/cosmos1mzr08swy3m6fx8xgmmfd5cf3wvjgsef8pnv9saacly03s3w4n5jq7qq9st)\
**Network:** Stargaze > Cosmos  Hub\
**Collection Type:** Burn-to-mint \
**Collection Size**: 1000\
**Royalties:**  5% \
Symbol: YWs&#x20;

### Yield Walkers Mint proceeds <a href="#pengones-mint-proceeds" id="pengones-mint-proceeds"></a>

Details of the PengOnes mint proceeds, allocation and base price to calculate the APRs.​

| NFT Quantity       | 1000 **YWs**         |
| ------------------ | -------------------- |
| Minted/Circulating | **1000 YWs**         |
| Mint Collection    | None ( Burn-to-mint) |
| Team Share         | **0%**               |

### Yield Walkers Burn info <a href="#pengones-burn-info" id="pengones-burn-info"></a>

Last updated 30 March, 2026.​

| Fresh Walker    | 550  | 6   | 544\| 99%   |
| --------------- | ---- | --- | ----------- |
| Rotting stalker | 250  | 0   | 250 \| 100% |
| Decayed Crawler | 125  | 0   | 125 \| 100% |
| Ancient Husk    | 75   | 0   | 75 \| 100%  |
| **Total**       | 1000 | 994 | 994\| 99%   |
